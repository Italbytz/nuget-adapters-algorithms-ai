...
public class FlexibleBacktrackingSolver<TVar,TVal> : AbstractBacktrackingSolver<TVar,TVal> where TVar : IVariable
{
    ...
    public override IAssignment<TVar, TVal>? Solve(ICSP<TVar, TVal> csp)
    {
        ...
        return log is { IsEmpty: false, InconsistencyFound: true } ? null : base.Solve(csp);
    }
    ...
    {
        var unassigned = csp.Variables.ToList().Where(v => !assignment.Contains(v)).ToList();
        return (variableSelectionStrategy != null)? variableSelectionStrategy.Apply(csp, unassigned).First() : unassigned.First();
